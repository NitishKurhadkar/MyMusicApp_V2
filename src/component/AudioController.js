...
function AudioController() {
  ...
  const timeUpdate = (event) => {
    if (audioEl.current.duration > 0) setIsLoading(false);

    if (!isSeeking) setSongPlayingTime(event.target.currentTime);
  };
    ...
    if (audioEl.current) {
      if (state.playStatus) {
        audioEl.current.play().then(() => {
          if (!state.playlist[state.currentSongIndex]?.songDuration) {
            axios.post("https://my-music-app-server-6yia.vercel.app/saveSongDuration", {
              name: state.playlist[state.currentSongIndex].name,
  ...
  return (
    <>
      {isShowSongCard ? (
        <SongDetails
          audioEl={audioEl}
          songTime={songTimeConvertInMinAndSec(songPlayingTime)}
          isShowSongCard={showSongCard}
        />
      ) : null}
      <div
        className="songContainer p-3 text-white w-100 d-flex justify-content-between"
        onClick={showSongCard}
      >
        {state.playlist !== undefined && state.playlist.length !== 0 ? (
          <audio
            ref={audioEl}
            src={state.playlist[state.currentSongIndex].url}
            onTimeUpdate={timeUpdate}
            onEnded={playNextSong}
          ></audio>
        ) : null}

        <div className="songNameDiv d-flex justify-content-center align-items-center">
          <img
          ...
          />
          <div className="ms-3">
            <p className="f-lato-r mb-0 fs-6 songNameText">
              {state.playlist[state.currentSongIndex].name}
            </p>
            <p className="f-14 f-lato-r mb-0">
              {state.playlist[state.currentSongIndex].artistName}
            </p>
          </div>
          {!isMobile() ? (
            state.likedSongPlaylist.some(
              ...
              <FavoriteIcon className="ms-3" onClick={dislikeSong} />
            ) : (
              <FavoriteBorderIcon className="ms-3" onClick={likeSong} />
            )
          ) : null}
        </div>
        <div className="d-flex flex-column justify-content-center w-30 audio_controller">
          ...
          {(isShowSongCard || !isMobile()) && (
            <div className="seekbar_container w-100">
              <label className="pe-3 f-14 f-lato-r">
                {songTimeConvertInMinAndSec(songPlayingTime)}
              </label>
        ...
        {!isMobile() ? (
          <div className="volume">
            ...
            <Slider
              ...
              max={1}
              step={0.01}
              onChange={(e, newValue) => (audioEl.current.volume = newValue)}
            />
            <label style={{ marginLeft: "5px" }}>
              {audioEl.current !== null
                ? parseInt(audioEl.current.volume * 100)
                : 100}
            </label>
          </div>
        ) : null}
