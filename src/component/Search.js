...
const Search = () => {
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(() => {
      if (inputSearch === "") {
        ...
        axios.get(`/search/${inputSearch}`).then((res) => {
          dispatch({
            type: "SEARCH_LIST",
  ...
  const addSongToPlaylist = (element) => {
    ...
    togglePlay(state, dispatch, element.id, 0, element.artist, {
      playlist: [element],
    });
  };
  ...
  return (
    <div className="px-3 pb-3 px-md-4 px-lg-4 mt-3 w-100 overflow-auto searchbox">
      <input
        ...
        value={inputSearch}
        className="position-fixed py-2 px-3 searchbar"
        placeholder="Search"
        type="text"
      />
      {inputSearch === "" ? (
        <p className="text-white position-absolute">Enter Search</p>
      ) : inputSearch !== "" ? (
                ...
                <div className="grid pt-1">
                  {state.searchSongList.map((song, index) => {
                    return (
                      <div
                        key={song.name + index}
                        className="p-3 rounded-3 audiocard"
                      >
                        <img
                        ...
                        />
                        <p
                          className="pt-2 text-white text-truncate mb-0"
                          title={song.name}
                        >
                          {song.name}
                        </p>
                    ...
                    return (
                      <div key={index} className="p-3 rounded-3 audiocard">
                        <img
                          ...
                          alt=""
                          onClick={() =>
                            openPlaylist("artist", artist.artistName)
                          }
                        />
                        <p
                          className="pt-2 text-white text-truncate mb-0"
                    ...
                    return (
                      <div key={index} className="p-3 rounded-3 audiocard">
                        <img
                          ...
                          src={movie.img}
                          alt=""
                          onClick={() => openPlaylist("movies", movie.name)}
                        />
                        <p
                          className="pt-2 text-white text-truncate mb-0"
                          title={movie.name}
